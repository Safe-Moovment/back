...
exports.createHealthBiometricData = createHealthBiometricData;
exports.createDeviceTelemetryData = createDeviceTelemetryData;
exports.createFenceRiskData = createFenceRiskData;
function assertNonNull(value, fieldName) {
    if (value === null || value === undefined) {
        throw new Error(`Missing required field: ${fieldName}`);
    }
    return value;
}
function createHealthBiometricData(payload) {
    return {
...
function createDeviceTelemetryData(payload) {
    return {
...
function createFenceRiskData(payload) {
    return {
        fence_id: assertNonNull(payload.fence_id, "fence_id"),
        status: assertNonNull(payload.status, "status"),
        geofence_polygon: assertNonNull(payload.geofence_polygon, "geofence_polygon").map((point, index) => ({
            lat: assertNonNull(point?.lat, `geofence_polygon[${index}].lat`),
            lng: assertNonNull(point?.lng, `geofence_polygon[${index}].lng`),
        })),
        terrain_risk_map: assertNonNull(payload.terrain_risk_map, "terrain_risk_map").map((risk, index) => ({
            coordinate: {
